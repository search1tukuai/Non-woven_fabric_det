...
def detect(opt):
    ...
    # source = "../../data/target/entire/0429-1.jpg"
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        for i, det in enumerate(pred):  # 检测每一张图片
            ...
            p = Path(p)  # 测试集文件地址
            save_path = str(save_dir / p.name)  # 文件保存地址
            txt_path = str(save_dir / 'labels' / p.stem) + ('' if dataset.mode == 'image' else f'_{frame}')  # img.txt
            s += '%gx%g ' % img.shape[2:]  # 图片大小
            ...
            if len(det):
                ...
                for c in det[:, -1].unique():
                    ...
                    s += f"{n} {names[int(c)]}{'s' * (n > 1)}, "  # 数量 标签名
...
if __name__ == '__main__':
    ...
    parser.add_argument('--weights', nargs='+', type=str, default='./model_data/yolov5xbest.pt', help='model.pt path(s)')  # 选择模型
    parser.add_argument('--source', type=str, default=r'D:\Download\test_data\jinsi', help='source')  # 测试集地址
